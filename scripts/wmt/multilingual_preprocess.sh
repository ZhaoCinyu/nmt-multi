...
multilingual_corpus_dir="/path/to/wmt"


root_data_dir=${project_dir}/data/wmt
...
for lang_pair in `ls ${multilingual_corpus_dir}`; do
    ...
    #if [[ ${tgt_lang} = "en" ]]; then
        spm_inputs+="${parallel_corpus_dir}/wmt.${src_lang}-${tgt_lang}-train.${src_lang},"
        spm_inputs+="${parallel_corpus_dir}/wmt.${src_lang}-${tgt_lang}-train.${tgt_lang},"
    #fi
...
for lang_pair in `ls ${multilingual_corpus_dir}`; do
    ...
    spm_encode --model ${spm_data_dir}/spm.model --output_format piece < ${parallel_corpus_dir}/wmt.${src_lang}-${tgt_lang}-train.${src_lang} > ${output_spm_parallel_corpus_dir}/wmt.${src_lang}-${tgt_lang}-train.${src_lang}
    spm_encode --model ${spm_data_dir}/spm.model --output_format piece < ${parallel_corpus_dir}/wmt.${src_lang}-${tgt_lang}-train.${tgt_lang} > ${output_spm_parallel_corpus_dir}/wmt.${src_lang}-${tgt_lang}-train.${tgt_lang}

    python -u ${project_dir}/nmt/data_handling/corpus_manager.py \
        --src_path ${output_spm_parallel_corpus_dir}/wmt.${src_lang}-${tgt_lang}-train.${src_lang} \
        --tgt_path ${output_spm_parallel_corpus_dir}/wmt.${src_lang}-${tgt_lang}-train.${tgt_lang} \
   ...
   for corpus_type in "dev" "test"; do
        ...
        if [[ -f ${parallel_corpus_dir}/wmt.${src_lang}-${tgt_lang}-${corpus_type}.${src_lang} ]] && [[ -f ${parallel_corpus_dir}/wmt.${src_lang}-${tgt_lang}-${corpus_type}.${tgt_lang} ]]; then
            spm_encode --model ${spm_data_dir}/spm.model --output_format piece < ${parallel_corpus_dir}/wmt.${src_lang}-${tgt_lang}-${corpus_type}.${src_lang} > ${output_spm_parallel_corpus_dir}/wmt.${src_lang}-${tgt_lang}-${corpus_type}.${src_lang}
            spm_encode --model ${spm_data_dir}/spm.model --output_format piece < ${parallel_corpus_dir}/wmt.${src_lang}-${tgt_lang}-${corpus_type}.${tgt_lang} > ${output_spm_parallel_corpus_dir}/wmt.${src_lang}-${tgt_lang}-${corpus_type}.${tgt_lang}

            # remove long sentences in dev set
            if [[ ${corpus_type} = "dev" ]]; then
                python -u ${project_dir}/nmt/data_handling/corpus_manager_wmt.py \
                    --src_path ${output_spm_parallel_corpus_dir}/wmt.${src_lang}-${tgt_lang}-${corpus_type}.${src_lang} \
                    --tgt_path ${output_spm_parallel_corpus_dir}/wmt.${src_lang}-${tgt_lang}-${corpus_type}.${tgt_lang} \
...
for lang_pair in `ls ${spm_corpus_dir}`; do
    ...
    if [[ -f ${spm_parallel_corpus}/wmt.${src_lang}-${tgt_lang}-test.${src_lang} ]] && [[ -f ${spm_parallel_corpus}/wmt.${src_lang}-${tgt_lang}-test.${tgt_lang} ]]; then
        options+="--testpref ${spm_parallel_corpus}/wmt.${src_lang}-${tgt_lang}-test"
    fi
